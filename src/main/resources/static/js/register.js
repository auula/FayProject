...
function validate() {
    ...
    if (!PwdReg.test($("#defaultLoginFormPassword1").val())) {
        $("#mess").html("密码最少6位，包括至少1个大写字母，1个小写字母，1个数字，1个特殊字符！！");
        return;
    }


    if ($("#defaultLoginFormPassword1").val() != $("#defaultLoginFormPassword2").val()) {
        $("#mess").html("2次密码不一致！！");
        return;
    }
    if($("#defaultLoginFormICode").val() == "") {
        $("#mess").html("邀请码不能为空！！你可以填写为0000!");
        return;
    }
    ...
    $("#mess").css("color","#0054ff").html("正在注册....");
    sendData();
}

function sendData() {
    $.ajax({
        url: domain + "/" + new Date().getTime() + "/user/register",
        type: "post",
        cache: false,
        async: false,
        dataType: "json",
        data: {
            ...
            "password": $("#defaultLoginFormPassword1").val(),
            "code": $("#defaultLoginFormText").val(),
            "icode": $("#defaultLoginFormICode").val(),
        },
